...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            //Мы ждем пока модель пользователя будет искать человека по email
            ...
            // ждем , пока сохраниться пользователь в БД
            await user.save()
...
router.post(
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (!isMatch) {
                return res.status(400).json({ message: "Не верный пароль, попробуй снова" })
            }
